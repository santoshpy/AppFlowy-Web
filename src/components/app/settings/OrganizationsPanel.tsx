import { useCallback, useEffect, useState } from 'react';
import { toast } from 'sonner';

import { OrganizationService } from '@/application/services/domains';
import { Organization, OrganizationMember } from '@/application/types';
import { ReactComponent as MoreIcon } from '@/assets/icons/more.svg';
import { useCurrentWorkspaceId } from '@/components/app/app.hooks';
import { Avatar, AvatarFallback } from '@/components/ui/avatar';
import { Button } from '@/components/ui/button';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu';
import { Input } from '@/components/ui/input';
import { Progress } from '@/components/ui/progress';
import { getErrorMessage } from '@/utils/errors';

const ORG_ROLE_ADMIN = 1;
const ORG_ROLE_MEMBER = 2;
// Backend permission-denied code (non-admins can't read/manage members).
const NOT_ENOUGH_PERMISSIONS = 1012;

export function OrganizationsPanel() {
  const currentWorkspaceId = useCurrentWorkspaceId();

  const [organizations, setOrganizations] = useState<Organization[]>([]);
  const [loadingOrgs, setLoadingOrgs] = useState(false);
  const [newName, setNewName] = useState('');
  const [creating, setCreating] = useState(false);

  const [selectedOrg, setSelectedOrg] = useState<Organization | null>(null);
  const [members, setMembers] = useState<OrganizationMember[]>([]);
  const [loadingMembers, setLoadingMembers] = useState(false);
  // True when the selected org's members can't be read — i.e. the current user
  // is a plain member, not an admin. Member management is hidden in that case.
  const [readOnly, setReadOnly] = useState(false);
  const [memberEmail, setMemberEmail] = useState('');
  const [addingMember, setAddingMember] = useState(false);
  const [attaching, setAttaching] = useState(false);

  // Bumped after a mutation to re-run the cancellable fetches below.
  const [orgsReloadKey, setOrgsReloadKey] = useState(0);
  const [membersReloadKey, setMembersReloadKey] = useState(0);

  useEffect(() => {
    let cancelled = false;

    setLoadingOrgs(true);
    void (async () => {
      try {
        const list = await OrganizationService.getOrganizations();

        if (!cancelled) setOrganizations(list);
      } catch (e) {
        if (!cancelled) toast.error(getErrorMessage(e));
      } finally {
        if (!cancelled) setLoadingOrgs(false);
      }
    })();

    return () => {
      cancelled = true;
    };
  }, [orgsReloadKey]);

  useEffect(() => {
    if (!selectedOrg) {
      setMembers([]);
      setReadOnly(false);
      return;
    }

    let cancelled = false;

    setLoadingMembers(true);
    setReadOnly(false);
    void (async () => {
      try {
        const list = await OrganizationService.getOrgMembers(selectedOrg.org_id);

        if (!cancelled) setMembers(list);
      } catch (e) {
        // A plain member can't read the roster: degrade to read-only instead of
        // a scary error toast.
        if ((e as { code?: number })?.code === NOT_ENOUGH_PERMISSIONS) {
          if (!cancelled) {
            setReadOnly(true);
            setMembers([]);
          }
        } else if (!cancelled) {
          toast.error(getErrorMessage(e));
        }
      } finally {
        if (!cancelled) setLoadingMembers(false);
      }
    })();

    return () => {
      cancelled = true;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedOrg, membersReloadKey]);

  const handleCreate = useCallback(async () => {
    const name = newName.trim();

    if (!name) return;
    setCreating(true);
    try {
      await OrganizationService.createOrganization({ name });
      toast.success('Department created');
      setNewName('');
      setOrgsReloadKey((k) => k + 1);
    } catch (e) {
      toast.error(getErrorMessage(e));
    } finally {
      setCreating(false);
    }
  }, [newName]);

  const handleAddMember = useCallback(async () => {
    const email = memberEmail.trim();

    if (!selectedOrg || !email) return;
    setAddingMember(true);
    try {
      await OrganizationService.addOrgMember(selectedOrg.org_id, { email, role: ORG_ROLE_MEMBER });
      toast.success('Member added');
      setMemberEmail('');
      setMembersReloadKey((k) => k + 1);
    } catch (e) {
      toast.error(getErrorMessage(e));
    } finally {
      setAddingMember(false);
    }
  }, [memberEmail, selectedOrg]);

  const handleSetRole = useCallback(
    async (member: OrganizationMember, role: number) => {
      if (!selectedOrg) return;
      try {
        await OrganizationService.addOrgMember(selectedOrg.org_id, { email: member.email, role });
        toast.success(role === ORG_ROLE_ADMIN ? 'Member promoted to admin' : 'Member role updated');
        setMembersReloadKey((k) => k + 1);
      } catch (e) {
        toast.error(getErrorMessage(e));
      }
    },
    [selectedOrg]
  );

  const handleRemoveMember = useCallback(
    async (uid: string) => {
      if (!selectedOrg) return;
      try {
        await OrganizationService.removeOrgMember(selectedOrg.org_id, uid);
        toast.success('Member removed');
        setMembersReloadKey((k) => k + 1);
      } catch (e) {
        toast.error(getErrorMessage(e));
      }
    },
    [selectedOrg]
  );

  const handleAttachWorkspace = useCallback(async () => {
    if (!selectedOrg || !currentWorkspaceId) return;
    setAttaching(true);
    try {
      await OrganizationService.attachWorkspaceToOrg(selectedOrg.org_id, currentWorkspaceId);
      toast.success('Current workspace added to the department');
    } catch (e) {
      toast.error(getErrorMessage(e));
    } finally {
      setAttaching(false);
    }
  }, [selectedOrg, currentWorkspaceId]);

  return (
    <div className='flex h-full min-h-0 flex-1 flex-col overflow-hidden'>
      <div className='border-b border-border-primary px-8 py-5'>
        <h2 className='text-xl font-semibold text-text-primary'>Departments</h2>
      </div>
      <div className='appflowy-scroller flex-1 overflow-y-auto px-8 py-6'>
        <div className='flex flex-col gap-6'>
          <div className='flex flex-col gap-2'>
            <div className='text-sm font-semibold text-text-primary'>Create a department</div>
            <div className='flex gap-2'>
              <Input
                className='flex-1'
                value={newName}
                placeholder='Department name (e.g. Ministry of Finance)'
                onChange={(e) => setNewName(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !creating && newName.trim()) {
                    void handleCreate();
                  }
                }}
                data-testid='org-name-input'
              />
              <Button
                onClick={() => void handleCreate()}
                disabled={!newName.trim() || creating}
                loading={creating}
                data-testid='org-create-button'
              >
                {creating && <Progress />}
                Create
              </Button>
            </div>
          </div>

          <div className='flex flex-col gap-2'>
            <div className='text-sm font-semibold text-text-primary'>Your departments</div>
            {loadingOrgs && organizations.length === 0 ? (
              <div className='py-4 text-center text-sm text-text-secondary'>
                <Progress />
              </div>
            ) : organizations.length === 0 ? (
              <div className='py-4 text-center text-sm text-text-secondary'>
                You don&apos;t belong to any departments yet
              </div>
            ) : (
              organizations.map((o) => (
                <button
                  key={o.org_id}
                  type='button'
                  data-testid={`org-select-${o.org_id}`}
                  onClick={() => setSelectedOrg(o)}
                  className={`flex items-center gap-3 rounded-300 px-3 py-2 text-left text-sm hover:bg-fill-content-hover ${
                    selectedOrg?.org_id === o.org_id ? 'bg-fill-content-hover' : ''
                  }`}
                >
                  <Avatar size='md'>
                    <AvatarFallback name={o.name}>{o.name.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <span className='truncate font-medium text-text-primary'>{o.name}</span>
                </button>
              ))
            )}
          </div>

          {selectedOrg && (
            <div className='flex flex-col gap-3 border-t border-border-primary pt-6'>
              <div className='flex items-center justify-between'>
                <div className='text-sm font-semibold text-text-primary'>
                  Members of {selectedOrg.name}
                </div>
                {!readOnly && (
                  <Button
                    variant='outline'
                    size='sm'
                    onClick={() => void handleAttachWorkspace()}
                    disabled={!currentWorkspaceId || attaching}
                    loading={attaching}
                    data-testid='org-attach-workspace-button'
                  >
                    Add current workspace
                  </Button>
                )}
              </div>

              {readOnly ? (
                <div className='py-4 text-center text-sm text-text-secondary'>
                  You&apos;re a member of this department. Only department admins can manage members.
                </div>
              ) : (
                <>
                  <div className='flex gap-2'>
                    <Input
                      className='flex-1'
                      value={memberEmail}
                      placeholder='Add a member by email'
                      onChange={(e) => setMemberEmail(e.target.value)}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && !addingMember && memberEmail.trim()) {
                          void handleAddMember();
                        }
                      }}
                      data-testid='org-member-email-input'
                    />
                    <Button
                      onClick={() => void handleAddMember()}
                      disabled={!memberEmail.trim() || addingMember}
                      loading={addingMember}
                      data-testid='org-member-add-button'
                    >
                      {addingMember && <Progress />}
                      Add
                    </Button>
                  </div>

                  {loadingMembers && members.length === 0 ? (
                    <div className='py-4 text-center text-sm text-text-secondary'>
                      <Progress />
                    </div>
                  ) : (
                    members.map((m) => (
                      <div
                        key={m.uid}
                        data-testid={`org-member-row-${m.uid}`}
                        className='flex items-center gap-3 py-2 text-sm'
                      >
                        <Avatar size='md'>
                          <AvatarFallback name={m.name}>
                            {(m.name || m.email).charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        <div className='flex min-w-0 flex-1 flex-col'>
                          <span className='truncate font-medium text-text-primary'>
                            {m.name || m.email}
                          </span>
                          <span className='truncate text-xs text-text-secondary'>{m.email}</span>
                        </div>
                        <span className='mr-1 text-xs text-text-secondary'>
                          {m.org_role === ORG_ROLE_ADMIN ? 'Admin' : 'Member'}
                        </span>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button
                              type='button'
                              data-testid={`org-member-actions-${m.uid}`}
                              className='flex h-6 w-6 items-center justify-center rounded-200 text-icon-secondary hover:bg-fill-content-hover'
                              aria-label='Member actions'
                            >
                              <MoreIcon className='h-4 w-4' />
                            </button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align='end'>
                            {m.org_role === ORG_ROLE_ADMIN ? (
                              <DropdownMenuItem
                                data-testid={`org-member-demote-${m.uid}`}
                                onSelect={() => void handleSetRole(m, ORG_ROLE_MEMBER)}
                              >
                                Make member
                              </DropdownMenuItem>
                            ) : (
                              <DropdownMenuItem
                                data-testid={`org-member-promote-${m.uid}`}
                                onSelect={() => void handleSetRole(m, ORG_ROLE_ADMIN)}
                              >
                                Make admin
                              </DropdownMenuItem>
                            )}
                            <DropdownMenuItem
                              variant='destructive'
                              data-testid={`org-member-remove-${m.uid}`}
                              onSelect={() => void handleRemoveMember(m.uid)}
                            >
                              Remove from department
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                    ))
                  )}
                </>
              )}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}

export default OrganizationsPanel;
