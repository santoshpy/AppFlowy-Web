...
// Brand identity, config-driven via APPFLOWY_BRAND_* env vars (defaults to AppFlowy).
// Mirrors src/application/brand.ts for the server-rendered marketing/publish pages.
const BRAND = {
  name: process.env.APPFLOWY_BRAND_NAME || 'AppFlowy',
  twitter: process.env.APPFLOWY_BRAND_TWITTER || '@appflowy',
};

const DEFAULT_DESCRIPTION = `Write, share, and publish docs quickly on ${BRAND.name}.\nGet started for free.`;
...
> = {
  '/after-payment': {
    title: `Payment Success | ${BRAND.name}`,
    description: `Payment success on ${BRAND.name}`,
  },
  '/login': {
    title: `Login | ${BRAND.name}`,
    description: `Login to ${BRAND.name}`,
  },
};
...
export const renderPublishPage = ({ hostname, pathname, metaData, publishError }: RenderPublishPageOptions) => {
  ...
  setOrUpdateMetaTag($, 'meta[property="og:site_name"]', 'property', BRAND.name);
  ...
  setOrUpdateMetaTag($, 'meta[name="twitter:site"]', 'name', BRAND.twitter);
